...
如果你想把自己的邮箱系统接进 `grok2api_register` 的 register stack，只要实现这里约定的接口，控制台和注册执行器就能直接工作。
...
- 不需要自己再实现这里这套自定义 Temp Mail 契约
- 把 `temp_mail_provider` 填成 `cloudmail`
- `temp_mail_api_base` 填 Cloud Mail 服务根地址
...
Cloud Mail 适配会按官方开放 API 调用：

1. `POST /api/public/genToken`，请求体为管理员邮箱和密码，返回身份令牌
2. `POST /api/public/addUser`，使用 `Authorization: <token>` 创建随机邮箱账号
3. `POST /api/public/emailList`，使用 `toEmail` 查询目标邮箱最近收件并提取验证码

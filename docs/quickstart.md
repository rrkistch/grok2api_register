...
如果你准备用 Cloud Mail，推荐填：

- `temp_mail_provider`: `cloudmail`
- `temp_mail_api_base`: 你的 Cloud Mail 服务根地址
- `temp_mail_admin_email`: Cloud Mail 管理员邮箱
- `temp_mail_admin_password`: Cloud Mail 管理员邮箱密码
- `temp_mail_domain`: 要创建账号的邮箱后缀
...
如果你已经使用根目录 `docker-compose.register.yml` 起整套服务：
